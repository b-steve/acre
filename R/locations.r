...
#'
#' @return a data frame with columns \code{x}, \code{y} and \code{z}
#' @export
#'
#' @examples
dist_density <- function(fit, call_id, animal_id=NULL, session=1, 
  ...
  for (i in seq_along(call_id)) {
    ...
    if (length(traps_used)) {
      
      # Keep only detectors with a non-missing recorded distance
      ok <- !is.na(dist_capt)
      ...
      if (length(traps_used)) {
        ...
        # All triggered traps had NA distances => no usable distance info
        z <- rep(1, nrow(mask))
      }
    } else {
  ...
  for (i in seq_along(call_id)) {
    ...
    if (length(traps_used)) {
      ...
      if (par_extended && is.matrix(kappa)) {
        dens_mat <- CircStats::dvm(bearing_capt[!is.na(bearing_capt)], 
                                   mu = active_bearings, 
...
#' @inheritParams location_density
#'
#' @return a data frame with columns \code{x}, \code{y} and \code{x}
#' @export
#'
#' @examples
