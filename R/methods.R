#' Extract coefficients from the output of acre_tmb model
#'
...
#'                       of 'x1' is not provided in the argument of 'new.covariates', then no matter what the 'types' is, these
...
coef.acre_tmb = function(object, types = NULL, pars = NULL, new.covariates = NULL, ...){
  ...
  for(i in pars){
    par_info = subset(df_param, df_param$par == i)
...
#' Title
#'
#' @param object an object generated by the bootstrap function "boot.acre()".
#' @param types a character vector, the same as "coef.acre_tmb()".
#' @param pars a character vector, the same as "coef.acre_tmb()".
#' @param new.covariates a data frame, the same as "coef.acre_tmb()".
  ...
  for(i in types){
    ...
    if(i == 'linked'){
      if(is.null(new.covariates)){
        ...
        #if new.covariates is provided, then we need to go each parameters one by one
        for(j in pars){
          values_link = coefs_linked_bias_corrected[original_name == j]
          par_info = subset(df_param, df_param$par == j)
          
          if(j %in% name_extend){
            if(!all(extend_par_covariates[[j]] %in% colnames(new.covariates))){
      ...
      coefs_esa = coef.acre_tmb(object, types = 'derived')
...
#' S3 method for output of coef.acre_tmb and coef.acre_boot
#'
#' @param x an object generated by the function "coef.acre_tmb()" or "coef.acre_boot()".
#' @param ... 
#'
#' @export
#'
print.coef_acre_tmb = function(x, ...){
  output = as.matrix(x)
  colnames(output) = "Est"
  print(output)
}
...
#' @export
vcov.acre_tmb = function(object, types = NULL, pars = NULL, new.covariates = NULL, show_fixed_par = TRUE, ...){
  ...
  #if new.covariates is provided, this "link_funs" will be useless as cov matrix of fitted and
  ...
  for(type in types){
    if(type == "linked"){
        ...
        for(p in pars){
          if(p %in% name_extend){
            if(p %in% name_extned_covariate_provided){
              tem = rep(paste0(p, "_link"), nrow(new.covariates))
            } else {
      ...
      #if 'new.covariates' is not provided, just keep the extended covariates as parameters with identity link function
      if(is.null(new.covariates)){
        output[[type]] = delta_method_acre_tmb(cov_linked, param_values, link_funs = link_funs, back_trans = TRUE)
...
vcov.acre_boot = function(object, types = NULL, pars = NULL, new.covariates = NULL, from_boot = TRUE, show_fixed_par = TRUE, ...){
  ...
  if(!from_boot){
    output = vcov.acre_tmb(object = object, types = types, pars = pars, new.covariates = new.covariates, show_fixed_par = show_fixed_par)
    return(output)
  }
  ...
  for(i in types){
    if(i == 'linked'){
      res_linked = res_transform(res, new.covariates, pars, object, back_trans = FALSE)
      ...
      res_fitted = res_transform(res, new.covariates, pars, object, back_trans = TRUE)
...
#' @return a named numeric vector
#' @export
stdEr.acre_tmb = function(object, types = NULL, pars = NULL, new.covariates = NULL, show_fixed_par = TRUE, ...){
  
  output_vcov = vcov.acre_tmb(object = object, types = types, pars = pars, new.covariates = new.covariates,
...
#'
#' @param object a fitted model from "fit.acre()".
#' @param parm a character vector, the same as "coef.acre_tmb()".
...
#' @return a matrix
#' @export
confint.acre_tmb = function(object, parm = NULL, level = 0.95, types = NULL, new.covariates = NULL, ...){
  #deal with default setting for 'types'
  tem = types_pars_sol(types, parm, new.covariates)
  types = tem$types
  pars = tem$pars
...
confint.acre_boot = function(object, parm = NULL, level = 0.95, types = NULL, new.covariates = NULL,
  ...
  if(!from_boot){
    ...
    output = confint.acre_tmb(object = object, types = types, level = level, pars = parm,
  ...
  for(i in types){
    if(i == 'linked'){
      ...
      res_esa = get_boot_res_esa(object)
      coef_esa = coef.acre_tmb(object, types = 'derived')
...
#'             the type of "fitted" in the function "coef.acre_tmb()" and the "link" correspondence to
#'             the type of "linked" in the function "coef.acre_tmb()".
#' @param newdata a data frame, the same as "coef.acre_tmb()". If there is any extended parameters, and
...
#'
#' @examples
predict.acre_tmb = function(fit, type = 'response', newdata = NULL, se.fit = TRUE, confidence = TRUE,
    ...
    if(n_row != 0) {
      ...
      if(se.fit){
        if(i %in% par_fixed_name){
          ...
          output[[i]]$StdError = as.vector(stdEr(fit, types = type, new.covariates = newdata, pars = i, show_fixed_par = FALSE))
        }
...
#' Title
#'
#' @param fit an object generated by the bootstrap function "boot.acre()".
#' @param type a character vector, the same as "predict.acre_tmb()".
#' @param newdata a data frame, the same as "predict.acre_tmb()".
#' @param se.fit a logical value indicates whether to show standard error.
#' @param confidence a logical value indicates whether to show confidence interval.
#' @param level a numeric value indicates the confident level, default is 0.95.
#' @param realnames a character vector, the same as "predict.acre_tmb()".
    ...
    if(n_row != 0){
      ...
      if(se.fit){
        ...
        if(i %in% par_fixed_name){
          ...
          output[[i]]$StdError = as.vector(stdEr(fit, types = type, new.covariates = newdata, pars = i, from_boot = from_boot, show_fixed_par = FALSE))
        }
...
#' Title
#'
#' @param object an object generated from the model fitting function "fit.acre_tmb()" or
