...
#loc.cov: a list or a data frame or a matrix. If it is a list, each element should be a data frame or a matrix
...
#where M is the number of mask points, and each row contains k points provided by "loc.cov" which are nearest to this mask point.
...
location_cov_to_mask = function(mask, loc.cov, control_nn2 = NULL, control_weight = NULL, control_char = NULL){
  ...
  if(is(loc.cov, 'data.frame') | is(loc.cov, 'matrix')){
    ...
    if("session" %in% colnames(loc.cov)){
      stopifnot(ncol(loc.cov) > 3)
      loc.cov = loc.cov[!duplicated(loc.cov[,c('session', 'x', 'y')]), ]
      name_cov = colnames(loc.cov)[-which(colnames(loc.cov) %in% c('session', 'x', 'y'))]
    } else {
      stopifnot(ncol(loc.cov) > 2)
    ...
    loc.cov = list(loc.cov)
  } else {
    stopifnot(is(loc.cov, 'list'))
    n_loc_cov = length(loc.cov)
  ...
  for(s in 1:n.sessions){
    ...
    for(i in 1:n_loc_cov){
      ...
      if(any(xlim_query[1] < xlim_data_max[1], xlim_query[2] > xlim_data_max[2],
        ...
        warning(paste0("for ", i, "'th element of 'loc.cov', the masks generated by the traps ",
...
sim_args_generator = function(sim_name){
  ...
  output$trap.cov = trap.cov
  output$loc.cov = loc.cov
...
predict_D_for_plot = function(fit, session_select = 1, new_data = NULL, D_cov = NULL, xlim = NULL, ylim = NULL,
  ...
  if('D' %in% get_par_extend_name(fit)){
    ...
    if(original_mask&is.null(convert.loc2mask)&is.null(D_cov)){
      ...
      #when we cannot find loc.cov from the model fitting object, it is possible the D is
...
convert_dist_cov_to_loc_cov = function(dist.cov, loc.cov, mask){
  ...
  if(!is.null(dist.cov)){
    ...
    if(is.null(loc.cov)){
      ...
      if(is(loc.cov, 'data.frame')){
        loc.cov = list(loc.cov)
        n_loc_cov = 1
      } else if(is(loc.cov, 'list')){
        n_loc_cov = length(loc.cov)
      } else {
